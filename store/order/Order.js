      ...
      'shopNowSettle': function (actionType, shoppingCartProduct) {      
        ...
        return new Promise((resolve, reject) => {
          ...
          if (!shoppingCartContainer.findProudctByProductItemId(productItemId)) {
            shoppingCartContainer.addProduct(shoppingCartProduct,function(){             
              self._getSettleInfo(actionType).then((settle) => {
                self._setContainer(settle.data);
                resolve(self);
              }).catch((info)=>{
      ...
      'useTicketSettle': function (actionType, userTicketId) {
        ...
        return new Promise((resolve, reject) => {
          self._getSettleInfo(actionType).then((settle) => {
            ...
            console.log(shoppingCartContainer.findProudctByProductItemId(productItemId));
            if (!shoppingCartContainer.findProudctByProductItemId(productItemId)) {
              let shoppingCartProduct = new ShoppingCartProduct(productInfo, { shopName: settle.data.shopName, shopId: settle.data.shopId });
              console.log(shoppingCartProduct);
              shoppingCartContainer.addProduct(shoppingCartProduct, function () { }, false);
            }
            resolve(self);
          }).catch((info) => {
            reject(info);
